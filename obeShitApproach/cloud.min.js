...
const to = 90;
const font = "Impact"
const text = "Java Java C C Java Java PHP Python Python"
...
function generate() {
    layout.font(font).spiral("archimedean"), fontSize = d3.scale["log"]().range([10, 100]), tags.length && fontSize.domain([+tags[tags.length - 1].value || 1, +tags[0].value]), complete = 0, statusText.style("display", null), words = [], layout.stop().words(tags.slice(0, max = tags.length)).start()
}
...
function draw(t, e) {
    ...
    .on("mouseover", function(d){
      if(!intro){
        clearInterval(anim)
        $('#anim_status').html("<span class='glyphicon glyphicon-pause'></span>")
      }
    })
    ...
    function e() {
        function t(t, n, a) {
            for (var r, o, s, l = ([{
                    ...
                    x: e[0],
                    y: e[1]
                }], n.x), i = n.y, d = Math.sqrt(e[0] * e[0] + e[1] * e[1]), h = m(e), f = 0.12345 < .5 ? 1 : -1, p = -f;
        ...
        return k.start = function() {
            function n() {
                for (var n, s = +""; + "" - s < w && ++u < o && z;) n = d[u], n.x = e[0] * (0.12345 + .5) >> 1, n.y = e[1] * (0.12345 + .5) >> 1, l(n, d, u), t(a, n, r) && (c.push(n), b.word(n), r ? i(r, n) : r = [{
                    x: n.x + n.x0,
                    y: n.y + n.y0
                }, {
                    x: n.x + n.x1,
...
d3.select("#random-palette").on("click", function() {
    ...
    function() {
        function t() {
            c = +angle_count, u = Math.max(-90, Math.min(90, +from)), i = Math.max(-90, Math.min(90, +to)), e()
        }
